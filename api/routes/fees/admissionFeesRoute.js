...
admissionFeesRouter.put('/:id',authenticationMiddleware ,async (req, res) => {
  try {
    ...
    if (!updatedProgram) return res.status(404).json({ success: false, error: 'Program not found' });
...
// DELETE
admissionFeesRouter.delete('/:id',authenticationMiddleware ,async (req, res) => {
  try {
    const deletedProgram = await AdmissionFees.findByIdAndDelete(req.params.id);
    if (!deletedProgram) return res.status(404).json({ success: false, error: 'Program not found' });
