...
router.get("/exam_jumblingcenters/file/view/:dbid", async (req, res) => {
    try {
        const fileDoc = await Model_ExamJumblingCenters.findById(
            req.params.dbid
        ).select({
            file: 1,
        });
...
router.get("/exam_jumblingcenters/file/download/:dbid", async (req, res) => {
    try {
        const fileDoc = await Model_ExamJumblingCenters.findById(
            req.params.dbid
        ).select({
...
router.post("/exam_jumblingcenters", async (req, res) => {
    ...
    try {
        ...
        const result = new Model_ExamJumblingCenters(fields);
